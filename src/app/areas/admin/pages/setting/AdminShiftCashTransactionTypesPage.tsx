...
import {  getInvnetoryMethodsListApi, getShiftCashTransactionTypesApi } from '../../../../../_poscommon/common/helpers/api_helpers/ApiCalls'


export default function AdminShiftCashTransactionTypesPage() {
    ...
    const [allCashTransactionTypesMethods, setAllCashTransactionTypesMethods] = useState<any>([]);
    ...
    useEffect(() => {
        getCashTransactionTypesListService();
    }, [listRefreshCounter]);

    const getCashTransactionTypesListService = () => {

        let pageBasicInfoParams = new URLSearchParams(pageBasicInfo).toString();
        

        getShiftCashTransactionTypesApi(pageBasicInfoParams)
            .then((res: any) => {
                const { data } = res;
                if (data && data.length > 0) {
                    ...
                    setAllCashTransactionTypesMethods(res?.data);

                }else{
                    setAllCashTransactionTypesMethods([]);
                }
    ...
    return (
        <AdminLayout>
            <AdminPageHeader
                title='Cash Transaction Types'
                pageDescription='Cash Transaction Types'
                            ...
                            >
                                <thead>
                                    <tr className='text-start text-muted fw-bolder fs-7 gs-0 bg-light'>
                                        <th colSpan={1} role="columnheader" className="min-w-125px ps-3 rounded-start" style={{ cursor: 'pointer' }}>Cash Transaction Type Id</th>
                                ...
                                <tbody className='text-gray-600 fw-bold'>
                                    ...
                                    {
                                        allCashTransactionTypesMethods != undefined && allCashTransactionTypesMethods.length > 0
                                            ?
                                            allCashTransactionTypesMethods?.map((record: any) => (
                                                <tr role='row'>
                                                    <td role="cell" className="ps-3">{record.cashTransactionTypeId}</td>
                                                    ...
                                                    <td role="cell" className="">{record.cashTransactionTypeName}</td>
