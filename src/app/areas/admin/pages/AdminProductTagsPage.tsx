...
import { getProductTagsApi, insertUpdateProductCategoryApi, insertUpdateProductTagApi } from '../../../../_poscommon/common/helpers/api_helpers/ApiCalls'
...
import TagsAddUpdateForm from '../components/productsCataglog/TagsAddUpdateForm'
...
export default function AdminProductTagsPage() {
    ...
    const [tagIdEdit, setTagIdEdit] = useState<number>(0);
    ...
    const [productAllTags, setProductAllTags] = useState<any>([]);
    const [searchFieldValues, setSearchFieldValues] = useState<{ [key: string]: string }>({});

    const HtmlSearchFields: HtmlSearchFieldConfig[] = [
        { inputId: 'tagIdSearch', inputName: 'tagIdSearch', labelName: 'Tag ID', placeHolder: 'Tag ID', type: 'number', defaultValue: '', iconClass: 'fa fa-search' },
        { inputId: 'tagNameSearch', inputName: 'tagNameSearch', labelName: 'Tag Name', placeHolder: 'Tag Name', type: 'text', defaultValue: '', iconClass: 'fa fa-search' },

    ];
    const [tagEditForm, setTagEditForm] = useState<any>(null); // Data of the category being edited
    ...
    const handleTagEditClick = (e: Event, id: number) => {
        
        e.preventDefault();
        const recordForEdit = productAllTags?.find((x: { tagId: number }) => x.tagId == id);
        setTagIdEdit(recordForEdit?.tagId);

        setTagEditForm({
            tagIdEditForm: recordForEdit?.tagId,
            tagName: recordForEdit?.tagName,
    ...
    const handleTagFormSubmit = (data: any) => {
        console.log('data tag: ', data); // Handle form submission here
        const { tagIdEditForm, tagName, isActive } = data;
        if (stringIsNullOrWhiteSpace(tagName) || stringIsNullOrWhiteSpace(isActive)) {
        ...
        let formData = {
            tagId: stringIsNullOrWhiteSpace(tagIdEditForm) ? 0 : tagIdEditForm,
            tagName: tagName,
            isActive: isActive?.toString() == "1" ? true : false,
        }
       

        insertUpdateProductTagApi(formData)
            .then((res: any) => {
    ...
    const getProductTagsService = () => {
        ...
        getProductTagsApi(pageBasicInfoParams)
            .then((res: any) => {
                const { data } = res;
                            ...
                            >
                                <thead>
                                    <tr className='text-start text-muted fw-bolder fs-7 gs-0 bg-light'>
                                        <th colSpan={1} role="columnheader" className="min-w-125px ps-3 rounded-start" style={{ cursor: 'pointer' }}>Tag Id</th>
                                        <th colSpan={1} role="columnheader" className="min-w-125px" style={{ cursor: 'pointer' }}>Tag Name</th>
                                ...
                                <tbody className='text-gray-600 fw-bold'>
                                    ...
                                    {
                                        productAllTags != undefined && productAllTags.length > 0
                                            ?
                                            productAllTags?.map((record: any) => (
                                                <tr role='row'>
                                                    <td role="cell" className="ps-3">{record.tagId}</td>
                                                    ...
                                                    <td>
                                                        <div className="d-flex align-items-center">
                                                            ...
                                                            <div className="ms-5">
                                                                ...
                                                                <a href="#" className="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="product_name">{record.tagName}</a>
                                                    ...
                                                    <td role="cell" className='text-end min-w-100px pe-3'>
                                                        <CommonTableActionCell
                                                            ...
                                                            deleteData={{
                                                                ...
                                                                entityRowId: record.tagId,
                                                                entityName: dBEntitiesConst.ProductTags.tableName,
                                                                entityColumnName: dBEntitiesConst.ProductTags.primaryKeyColumnName,
